...
async function getFullState(userId) {
  return dbGet(
    `SELECT u.id, u.email, u.current_tier, u.day_in_tier, u.player_name,
            u.pending_tax, u.pending_grain, u.pending_buy,
            ps.city_name, ps.population, ps.treasury, ps.grain_stored, ps.public_anger,
            ps.growth_streak, ps.happy_streak, ps.next_grain_price,
...
async function issueNewAssignment(userId, tier, carryTreasury, reason) {
  ...
  await dbRun(
    `UPDATE users SET pending_tier = ?, pending_treasury = ?,
     pending_tax = NULL, pending_grain = NULL, pending_buy = 0 WHERE id = ?`,
...
async function saveState(state) {
  await dbRun(
    `UPDATE users SET current_tier = ?, day_in_tier = ?,
                      pending_tax = NULL, pending_grain = NULL, pending_buy = 0
     WHERE id = ?`,
  ...
  await dbRun(
    ...
    [state.city_name, state.population, state.treasury, state.grain_stored,
     ...
     state.next_grain_price, state.id]
  );
}
  ...
  for (const { id } of players) {
    try {
      ...
      if (state.day_in_tier === 1 && state.pending_tax === null) {
        ...
        await dbRun(
          `UPDATE player_states SET next_grain_price = ? WHERE user_id = ?`,
          [firstPrice, state.id]
        );
        await sendYearOneDispatch(state.email, state, firstPrice);
        console.log(`[dispatch] ${state.email} — Year I initial dispatch sent (price=${firstPrice})`);
        continue;
      }

      // Use the price that was shown to the player in the last email
      ...
      // Roll next turn's price now so it can be shown in the email
      const nextGrainPrice = rollGrainPrice();
      updated.next_grain_price = nextGrainPrice;

      await sendDispatch(state.email, state, updated, orders, grainPrice, nextGrainPrice);
